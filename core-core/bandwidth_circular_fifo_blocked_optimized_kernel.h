    ...
    #define FIFO_CONTENT_WRAPAROUND (2048)
    ...
    size_t fifoOptimizedAllocate(elementType* &shared_array_loc, elementType* &local_array_reader_loc, elementType* &local_array_writer_loc, atomicIndexType* &shared_write_id_loc, atomicIndexType* &shared_read_id_loc, int array_length, int block_length, int cpu_a, int cpu_b){
        ...
        local_array_reader_loc = (elementType*) aligned_alloc_core(CACHE_LINE_SIZE, amountToAlloc, cpu_b);
        local_array_writer_loc = (elementType*) aligned_alloc_core(CACHE_LINE_SIZE, amountToAlloc, cpu_a);
