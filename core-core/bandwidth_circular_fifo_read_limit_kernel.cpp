    ...
    *write_pos_shared_ptr = write_id; //sync write id
    ...
    while(write_id < STIM_LEN)
    {
        ...
        if((write_id - read_id) < length)
        {
            ...
            int32_t number_to_write = (space_in_queue > max_elements_per_transaction) ? max_elements_per_transaction : space_in_queue;
    ...
    *read_pos_shared_ptr = read_id; //sync read id
    ...
    while(read_id < STIM_LEN)
    {
        ...
        if(write_id > read_id)
        {
            ...
            int32_t num_avil_to_read = write_id - read_id;

            int32_t num_to_read = (num_avil_to_read > max_elements_per_transaction) ? max_elements_per_transaction : num_avil_to_read;
