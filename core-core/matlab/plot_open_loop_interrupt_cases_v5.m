    ...
    if PlotAll || ~isempty(find(endTrackerStdServer)) || ...
        ...
        if Summary
            title({'End Trace', filterCond, ['Transactions: ' num2str(TransactionCore_Client(idx))], ...
                                            ['Writer Trial Interrupts: ' num2str(endTrackerStdServer + endTrackerLocServer + endTrackerOtherArchServer + endTrackerSoftirqServer)], ...
                                            ['Reader Trial Interrupts: ' num2str(endTrackerStdClient + endTrackerLocClient + endTrackerOtherArchClient + endTrackerSoftirqClient)]});
        else
            title({'End Trace', filterCond, ['Transactions: ' num2str(TransactionCore_Client(idx))], ...
                                            ['Writer Trial Interrupts (In Window): ' num2str(sum(endTrackerStdServer) + sum(endTrackerLocServer) + sum(endTrackerOtherArchServer) + sum(endTrackerSoftirqServer))], ...
                                            ['Reader Trial Interrupts (In Window): ' num2str(sum(endTrackerStdClient) + sum(endTrackerLocClient) + sum(endTrackerOtherArchClient) + sum(endTrackerSoftirqClient))]});
        end
        ...
        if ~Summary
            ...
            ax3 = subplot(4, 1, 3);
            if PlotThickLines
                plot(ax2, endTrackerStdServer,        '-', 'LineWidth', 8)
                hold (ax2, 'on');
                plot(ax2, endTrackerLocServer,        '-', 'LineWidth', 6)
                plot(ax2, endTrackerOtherArchServer,  '-', 'LineWidth', 4)
                plot(ax2, endTrackerSoftirqServer,    '-', 'LineWidth', 2)
                hold (ax2, 'off');
                plot(ax3, endTrackerStdClient,        '-', 'LineWidth', 8)
                hold (ax3, 'on');
                plot(ax3, endTrackerLocClient,        '-', 'LineWidth', 6)
                plot(ax3, endTrackerOtherArchClient,  '-', 'LineWidth', 4)
                plot(ax3, endTrackerSoftirqClient,    '-', 'LineWidth', 2)
                hold (ax3, 'off');
            else
                ...
                plot(ax2, endTrackerSoftirqServer)
                hold (ax2, 'off');
                plot(ax3, endTrackerStdClient)
                hold (ax3, 'on');
                plot(ax3, endTrackerLocClient)
                plot(ax3, endTrackerOtherArchClient)
                plot(ax3, endTrackerSoftirqClient)
                hold (ax3, 'off');
            end
            hold off;
            legend(ax2, {'Writer Std', 'Writer LOC', 'Writer Other Arch', 'Writer softirq'}, ...
                    'Location', 'northwest')
            ylabel(ax2, 'Interrupts');
            xlabel(ax2, 'Iterations');
            
            legend(ax3, {'Reader Std', 'Reader LOC', 'Reader Other Arch', 'Reader softirq'}, ...
                    'Location', 'northwest')
