...
figure;
subplot(2, 1, 1);
loglog(single_array_length, single_latency_avg);
xlim([0,66]);
...
loglog(single_array_length, single_data_rate);
xlim([0,66]);
...
figure;
subplot(2, 1, 1);
loglog(dual_array_length, dual_latency_avg);
xlim([0,66]);
xlabel('Transaction Length (int32_t Elements)', 'Interpreter', 'none');
ylabel('Transaction Latency - One Way (ns)')
...
loglog(dual_array_length, dual_data_rate);
xlim([0,66]);
ylabel('Throughput (Mbps)');
xlabel('Transaction Length (int32_t Elements)', 'Interpreter', 'none');
grid on;
...
figure;
subplot(2, 1, 1);
loglog(fifo_array_length, fifo_latency_avg);
xlim([0,66]);
ylim([10, 1000]);
xlabel('Transaction Length (int32_t Elements)', 'Interpreter', 'none');
...
loglog(fifo_array_length, fifo_data_rate);
xlim([0,66]);
ylabel('Throughput (Mbps)');
xlabel('Transaction Length (int32_t Elements)', 'Interpreter', 'none');
grid on;
